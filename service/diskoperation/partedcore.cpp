...
void PartedCore::set_device_partitions(Device &device, PedDevice *lp_device, PedDisk *lp_disk)
{
    ...
    while (lp_partition) {
        ...
        delete partition_temp;
        partition_temp = nullptr;
    }   
...
bool PartedCore::deletePartition(const QString &devicePath, const QString &parttitionPath)
{
    ...
    if (!get_device_and_disk(devicePath, lpDevice, lpDisk)) {
    ...
    if (ped == nullptr) {
        ...
        destroy_device_and_disk(lpDevice, lpDisk);
...
bool PartedCore::hidePartition(const QString &devicePath, const QString &parttitionPath)
{
    ...
    int hideFlag = ped_partition_get_flag(ped, PED_PARTITION_HIDDEN);
    if (hideFlag == 1) {
